...
pub fn render_params_module_test() {
  let analyzed = analyzed_queries("test/fixtures/query.sql")
  let rendered = codegen.render_params_module(analyzed)
...
pub fn render_models_module_test() {
  let analyzed = analyzed_queries("test/fixtures/query.sql")
  let rendered = codegen.render_models_module(analyzed)
...
pub fn render_pog_adapter_test() {
  ...
  let analyzed = analyzed_queries("test/fixtures/query.sql")
  let rendered = codegen.render_adapter_module(block, analyzed)
...
pub fn render_sqlight_adapter_test() {
  ...
  let assert Ok(queries) =
    query_parser.parse_file("test/fixtures/query.sql", model.SQLite, content)
  let analyzed = query_analyzer.analyze_queries(model.SQLite, catalog, queries)
  let rendered = codegen.render_adapter_module(block, analyzed)
...
fn analyzed_queries(path: String) -> List(model.AnalyzedQuery) {
  ...
  let assert Ok(queries) =
    query_parser.parse_file(path, model.PostgreSQL, content)
